...
const Game2048 = () => {
    ...
    const [board, setBoard] = useState(Array(4).fill().map(() => Array(4).fill(null)));
    useEffect(() => {
        resetGame();
    ...
    function updateBoard(Board) {
        ...
        for (const row of Board) {
            ...
            for (let i = 0; i < nonNullCells.length; i++) {
                if (i < nonNullCells.length - 1 && nonNullCells[i] === nonNullCells[i + 1]) {
    ...
    const resetGame = () => {
        const newBoard = Array(4).fill().map(() => Array(4).fill(null));
                ...
                {board.map((row, rowIndex) => (
                    row.map((tile, colIndex) => (
                        <div
                            ...
                            font-medium 
                            ${tile === 2 ? 'bg-amber-50 text-neutral-950'
                                    : tile === 4 ? 'bg-amber-100 text-neutral-900'
                                        : tile === 8 ? 'bg-amber-200 text-neutral-800'
                                            : tile === 16 ? 'bg-amber-300 text-neutral-900'
                                                : tile === 32 ? 'bg-amber-400 text-neutral-900'
                                                    : tile === 64 ? 'bg-amber-500 text-neutral-900'
                                                        : tile === 128 ? 'bg-amber-600 text-neutral-900'
                                                            : tile === 256 ? 'bg-amber-700 text-neutral-50'
                                                                : tile === 512 ? 'bg-amber-800 text-neutral-200'
                                                                    : tile === 1024 ? 'bg-amber-900 text-neutral-100'
                                                                        : tile === 2048 ? 'bg-amber-950 text-white'
                                                                            : tile === null && 'bg-white'
                                }`}
                        >
                            {tile}
